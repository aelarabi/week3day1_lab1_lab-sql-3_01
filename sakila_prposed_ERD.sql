...
CREATE TABLE `film_category` (
  `film_catgory_id` SMALLINT PRIMARY KEY NOT NULL AUTO_INCREMENT,
  `film_id` SMALLINT NOT NULL,
...
CREATE INDEX `idx_fk_amount` ON `category` (`amount`);
...
ALTER TABLE `category` ADD FOREIGN KEY (`amount`) REFERENCES `payment` (`amount`) ON DELETE RESTRICT ON UPDATE CASCADE;

ALTER TABLE `film` ADD FOREIGN KEY (`amount`) REFERENCES `payment` (`amount`) ON DELETE RESTRICT ON UPDATE CASCADE;

ALTER TABLE `film_actor` ADD FOREIGN KEY (`amount`) REFERENCES `payment` (`amount`) ON DELETE RESTRICT ON UPDATE CASCADE;
